...
Demo GitHub Pages: [simple-todo-bmad-claude](https://cloly1941.github.io/simple-todo-bmad-claude/)
...
- CI/CD build, test và deploy lên GitHub Pages bằng GitHub Actions.
...
- [GitHub Actions](https://github.com/features/actions) – Chạy test, build và deploy tự động.
- [GitHub Pages](https://pages.github.com/) – Hosting static site production.
...
├── .github/
│   └── workflows/
│       └── deploy.yml          # CI/CD: test, build và deploy GitHub Pages
...
- `vite.config.js` cấu hình `base: '/simple-todo-bmad-claude/'` để chạy đúng trên GitHub Pages.
- GitHub Actions chạy `npm ci`, `npm test`, `npm run build` trên pull request và push.
- Deploy GitHub Pages chỉ chạy khi push lên branch `main`.
- Test script dùng `src/*.test.js` để tương thích shell trên môi trường Linux CI.
